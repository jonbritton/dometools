...
"""
make7th.py  –  Convert PNG/EXR frame sequences to .7TH-422 files

Usage:
    ./make7th.py /frames/            /output/dir/
    ./make7th.py '/frames/*.exr'     /output/dir/
        ...
        .7th files as needed. Output names are derived from the input sequence:
            show.seq01.4096x4096.00001.exr … show.seq01.4096x4096.00029.exr
          →  show.seq01.4096x4096.00000.7th  show.seq01.4096x4096.00010.7th  show.seq01.4096x4096.00020.7th
...
# EXR support requires:  pip install openexr imath
try:
    import OpenEXR
    import Imath
...
# Cb = (B - Y) / (2·(1-Kb)) + 0.5
# Cr = (R - Y) / (2·(1-Kr)) + 0.5
# NOTE: analysis of DFM reference files suggests BT.601 (0.299/0.587/0.114)
# may be a closer color-math match, but switching broke player compatibility
# in testing — leaving as BT.709 until further investigation.
...
def load_exr(path):
    ...
    quantisation in rgb_float_to_yuyv().  
    For scene-linear ACES content apply a viewing LUT / exposure adjustment before passing to this.
    """
    if not HAS_OPENEXR:
        raise RuntimeError(
...
#
# Matches the trailing frame number in names like "show.seq01.4096x4096.00042.png"
...
def parse_frame_number(path):
    ...
    e.g. "show.seq01.4096x4096.00042" -> (42, 5, "show.seq01.4096x4096.")
    """
...
def main():
    ...
    for idx, (cs, chunk) in enumerate(sorted(chunks.items()), 1):
        ...
        # has changed since it was last recorded in the manifest.
        ...
        with out_path.open("wb") as out:
            ...
            # If a chunk is partial (e.g., last chunk of a shot whose length
            # isn't a multiple of 10), fill the remaining slots with a copy
            # of the last real frame rather than black.  This avoids a visible
            # black flash at loop boundaries.
            last_frame = None
